...
router.delete('/experience/:exp_id', authorize, async (req, res) => {
  try {
    ...
    profile.experience.splice(removeIndex, 1); // remove 1 element at index removeIndex
    await profile.save();
    res.json(profile);
  } catch (err) {
    console.error(err.message);
...
router.delete('/education/:edu_id', authorize, async (req, res) => {
  try {
    ...
    profile.education.splice(removeIndex, 1); // remove 1 element at index removeIndex
    await profile.save();
    res.json(profile);
  } catch (err) {
    console.error(err.message);
